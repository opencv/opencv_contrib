...
- **Exercise the fix through the public `cv::linemod::Detector` API only — do not `#include` `linemod.cpp` directly.** An earlier version of this plan proposed a `#include "../src/linemod.cpp"` fallback for reaching the private `spread()`/`orUnaligned8u()` internals, believing it mirrored an existing pattern elsewhere in opencv_contrib. Code review (two independent reviewers) found that belief was based on a misread: the cited precedents (`modules/cvv/test/test_reference.cpp`, `modules/superres/test/test_superres.cpp`, `modules/cudafilters/test/test_filters.cpp`) include header (`.h`/`.hpp`) files, not `.cpp` translation units. `linemod.cpp` additionally defines out-of-line, external-linkage member bodies for `CV_EXPORTS_W`-declared classes (`Detector::match`, `ColorGradient::read/write`, etc.) already compiled into the `opencv_rgbd` module library that `opencv_test_rgbd` links against — including the whole file would redefine those symbols, which is a hard MSVC compile error (dllimport redefinition) and a duplicate-symbol link error elsewhere. Confirmed via `Detector::match()`'s own body (`modules/rgbd/src/linemod.cpp` ~line 1389-1444): it unconditionally rebuilds its linear-memory pyramid — and therefore unconditionally calls `spread()` — on every `match()` call, regardless of whether any template was added, and `ColorGradientPyramid::quantize()` (~line 478-482) sets the quantized `Mat`'s size directly from the source image's size with no row-width padding. This means a source image whose width is not a multiple of 16 reaches the exact stride-drift bug via a single `Detector::match()` call — no internals access needed, and it exercises the real crash path end-to-end (construction → template extraction → matching) rather than an isolated internal function.
- Construct a synthetic `CV_8UC3` checkerboard source image sized so its row width is not a multiple of 16 at every pyramid level `Detector` builds (level 0 = raw size, level 1 = `pyrDown()`'d to roughly half), while `rows*cols` stays a multiple of 16 at each level to satisfy `computeResponseMaps()`'s unrelated `CV_Assert((src.rows*src.cols) % 16 == 0)`. Use lenient `ColorGradient` thresholds (via `ColorGradient::create`, not the zero-arg constructor/`getDefaultLINE()`'s stricter defaults) so template extraction succeeds reliably on the synthetic pattern. Add one template via `addTemplate`, then call `match()` on the same image and assert a real match is found with high similarity — not just "did not crash" — since a SIGSEGV terminates the test process outright (a real regression signal CI already detects), but a silent wrong-output regression would not.
