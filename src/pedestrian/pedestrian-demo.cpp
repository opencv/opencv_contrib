...
int main(int argc, char **argv) {
    ...
    while (true) {
        ...
        hog.detectMultiScale(videoFrameDownGrey, locations, 0, cv::Size(), cv::Size(), 1.05, 2.0, false);
        maxLocations.clear();
        if (!locations.empty()) {
            boxes.clear();
            probs.clear();
            ...
            vector<bool> keep = non_maximal_suppression(&boxes, &probs);
