...
opencv_contrib_path = "/Users/virt/Downloads/opencv_contrib-master/modules" 
...
def build_opencv(srcroot, buildroot, target, arch):
    ...
    if opencv_contrib_path is not None:
        args = (" -DCMAKE_SKIP_INSTALL_ALL_DEPENDENCY=OFF " + 
                     ...
                     " -DBUILD_opencv_xphoto=OFF ") % opencv_contrib_path
        cmakeargs += args
        build_target = "opencv_xfeatures2d"
        libname = "libopencv_xfeatures2d.a"
    else:
    ...
    execute("xcodebuild IPHONEOS_DEPLOYMENT_TARGET=7.0 -parallelizeTargets ARCHS=%s -jobs 8 -sdk %s -configuration Release -target %s" % (arch, target.lower(), build_target))
    execute("xcodebuild IPHONEOS_DEPLOYMENT_TARGET=7.0 ARCHS=%s -sdk %s -configuration Release -target install install" % (arch, target.lower()))
    os.chdir(currdir)
...
def put_framework_together(srcroot, dstroot):
    ...
    libname = "libopencv_world.a" if opencv_contrib_path is None else "libopencv_xfeatures2d.a"
