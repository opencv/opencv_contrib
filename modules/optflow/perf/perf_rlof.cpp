...
namespace opencv_test {
	namespace {
		...
		{
			Size sz(640,480);
			string frame1_path = "B:/workspace/opencv_extra/testdata/cv/optflow/RubberWhale1.png";
			string frame2_path = "B:/workspace/opencv_extra/testdata/cv/optflow/RubberWhale2.png";
			...
			if (get<1>(GetParam()) == "SR_FIXED")
				param.supportRegionType = RLOFOpticalFlowParameter::SR_FIXED;
		...
		)
		{
			...
			if (get<0>(GetParam()) == "INTERP_EPIC")
				interp_type = DenseRLOFOpticalFlow::InterpolationType::INTERP_EPIC;
			if (get<0>(GetParam()) == "INTERP_GEO")
				interp_type = DenseRLOFOpticalFlow::InterpolationType::INTERP_GEO;
