...
namespace kinfu
{
TSDFVolume::TSDFVolume(Point3i _res, float _voxelSize, Affine3f _pose, float _truncDist,
                       int _maxWeight, float _raycastStepFactor, bool zFirstMemOrder)
      ...
      raycastStepFactor(_raycastStepFactor)
{
    ...
    neighbourCoords =
        Vec8i(volDims.dot(Vec4i(0, 0, 0)), volDims.dot(Vec4i(0, 0, 1)), volDims.dot(Vec4i(0, 1, 0)),
              volDims.dot(Vec4i(0, 1, 1)), volDims.dot(Vec4i(1, 0, 0)), volDims.dot(Vec4i(1, 0, 1)),
              volDims.dot(Vec4i(1, 1, 0)), volDims.dot(Vec4i(1, 1, 1)));
}

// dimension in voxels, size in meters
TSDFVolumeCPU::TSDFVolumeCPU(Point3i _res, float _voxelSize, cv::Affine3f _pose, float _truncDist,
                             int _maxWeight, float _raycastStepFactor, bool zFirstMemOrder)
    : TSDFVolume(_res, _voxelSize, _pose, _truncDist, _maxWeight, _raycastStepFactor,
                 zFirstMemOrder)
{
...
Voxel TSDFVolumeCPU::at(const cv::Vec3i& volumeIdx) const
{
    //! Out of bounds
    ...
    /* std::cout << " Current TSDF: " << volData[coordBase].tsdf << " current weight: " <<
     * volData[coordBase].weight << "\n"; */
...
static inline depthType bilinearDepth(const Depth& m, cv::Point2f pt)
{
    ...
    if (pt.x < 0 || pt.x >= m.cols - 1 || pt.y < 0 || pt.y >= m.rows - 1)
        return defaultValue;
...
{
    IntegrateInvoker(TSDFVolumeCPU& _volume, const Depth& _depth, Intr intrinsics,
                     cv::Affine3f cameraPose, float depthFactor)
        : ParallelLoopBody(),
          volume(_volume),
          depth(_depth),
    ...
    virtual void operator()(const Range& range) const override
    {
        ...
        Point3f zStepPt =
            Point3f(vol2cam.matrix(0, 2), vol2cam.matrix(1, 2), vol2cam.matrix(2, 2)) *
            volume.voxelSize;
        ...
        for (int x = range.start; x < range.end; x++)
        {
            ...
            for (int y = 0; y < volume.volResolution.y; y++)
            {
                ...
                /* std::cout << "StartZ, EndZ: " << startZ << ", " << endZ << "\n"; */
                for (int z = startZ; z < endZ; z++)
                {
                    ...
                    float zCamSpace =
                        v_reinterpret_as_f32(v_rotate_right<2>(v_reinterpret_as_u32(camSpacePt)))
                            .get0();
                    /* std::cout << "zCamSpace: " << zCamSpace << std::endl; */
                    if (zCamSpace <= 0.f)
                        continue;
                    ...
                    /* std::cout << "SDF: " << sdf << "truncDist: " << -volume.truncDist << "\n"; */
        ...
        for (int x = range.start; x < range.end; x++)
        {
            ...
            for (int y = 0; y < volume.volResolution.y; y++)
            {
                ...
                for (int z = startZ; z < endZ; z++)
                {
                    ...
                    /* std::cout << "camSpacePt: " << camSpacePt << "\n"; */
...
inline v_float32x4 TSDFVolumeCPU::getNormalVoxel(const v_float32x4& p) const
{
    if (v_check_any((p < v_float32x4(1.f, 1.f, 1.f, 0.f)) +
                    (p >= v_float32x4((float)(volResolution.x - 2), (float)(volResolution.y - 2),
...
inline Point3f TSDFVolumeCPU::getNormalVoxel(Point3f p) const
{
    ...
    if (p.x < 1 || p.x >= volResolution.x - 2 || p.y < 1 || p.y >= volResolution.y - 2 || p.z < 1 ||
        p.z >= volResolution.z - 2)
        return nan3;
...
struct RaycastInvoker : ParallelLoopBody
{
    RaycastInvoker(Points& _points, Normals& _normals, Affine3f cameraPose, Intr intrinsics,
        ...
        : ParallelLoopBody(),
          ...
          boxMax(volume.volSize - Point3f(volume.voxelSize, volume.voxelSize, volume.voxelSize)),
          boxMin(),
    ...
    virtual void operator()(const Range& range) const override
    {
        ...
        const v_float32x4 invVoxelSize =
            v_float32x4(volume.voxelSizeInv, volume.voxelSizeInv, volume.voxelSizeInv, 1.f);
            ...
            for (int x = 0; x < points.cols; x++)
            {
                ...
                if (tmin < tmax)
                {
                    ...
                    for (; steps < nSteps; steps++)
                    {
                        ...
                        fnext = volume.volume.at<Voxel>(coord).tsdf;
                        if (fnext != f)
                        {
                    ...
                    if (f > 0.f && fnext < 0.f)
                    {
                        ...
                        if (!cvIsNaN(ts) && !cvIsInf(ts))
                        {
                            ...
                            if (!isNaN(nv))
                            {
                                ...
                                normal =
                                    v_matmuladd(nv, volRot0, volRot1, volRot2, v_setzero_f32());
                                // interpolation optimized a little
                                point =
                                    v_matmuladd(pv * v_float32x4(volume.voxelSize, volume.voxelSize,
    ...
    virtual void operator()(const Range& range) const override
    {
        ...
        for (int y = range.start; y < range.end; y++)
        {
            ...
            for (int x = 0; x < points.cols; x++)
            {
                ...
                // re-order intersections to find smallest and largest on each axis
                Point3f minAx(min(ttop.x, tbottom.x), min(ttop.y, tbottom.y),
                              min(ttop.z, tbottom.z));
                Point3f maxAx(max(ttop.x, tbottom.x), max(ttop.y, tbottom.y),
                              max(ttop.z, tbottom.z));
                ...
                if (tmin < tmax)
                {
                    ...
                    for (; steps < nSteps; steps++)
                    {
                        ...
                        fnext = volume.volume.at<Voxel>(ix * xdim + iy * ydim + iz * zdim).tsdf;
                        if (fnext != f)
                        {
                            fnext = volume.interpolateVoxel(next);
...
{
    FetchPointsNormalsInvoker(const TSDFVolumeCPU& _volume, std::vector<std::vector<ptype>>& _pVecs,
                              std::vector<std::vector<ptype>>& _nVecs, bool _needNormals)
        : ParallelLoopBody(), vol(_volume), pVecs(_pVecs), nVecs(_nVecs), needNormals(_needNormals)
    {
        volDataStart = vol.volume.ptr<Voxel>();
    }

    inline void coord(std::vector<ptype>& points, std::vector<ptype>& normals, int x, int y, int z,
                      Point3f V, float v0, int axis) const
    {
        ...
        {
            const Voxel& voxeld =
                volDataStart[(x + shift.x) * vol.volDims[0] + (y + shift.y) * vol.volDims[1] +
            ...
            if (voxeld.weight != 0 && vd != 1.f)
            {
                if ((v0 > 0 && vd < 0) || (v0 < 0 && vd > 0))
                {
                    ...
                    Point3f p(shift.x ? inter : V.x, shift.y ? inter : V.y, shift.z ? inter : V.z);
                    {
        ...
        for (int x = range.start; x < range.end; x++)
        {
            ...
            for (int y = 0; y < vol.volResolution.y; y++)
            {
                ...
                for (int z = 0; z < vol.volResolution.z; z++)
                {
                    ...
                    {
                        Point3f V(Point3f((float)x + 0.5f, (float)y + 0.5f, (float)z + 0.5f) *
                                  vol.voxelSize);
...
#ifdef HAVE_OPENCL
TSDFVolumeGPU::TSDFVolumeGPU(Point3i _res, float _voxelSize, cv::Affine3f _pose, float _truncDist,
...
// use depth instead of distance (optimization)
void TSDFVolumeGPU::integrate(InputArray _depth, float depthFactor, cv::Affine3f cameraPose,
                              Intr intrinsics)
{
    CV_TRACE_FUNCTION();
    ...
    k.args(ocl::KernelArg::ReadOnly(depth), ocl::KernelArg::PtrReadWrite(volume),
           ocl::KernelArg::Constant(vol2cam.matrix.val, sizeof(vol2cam.matrix.val)), voxelSize,
           volResGpu.val, volDims.val, fxy.val, cxy.val, dfac, truncDist, maxWeight);
                            ...
                            cv::OutputArray _points, cv::OutputArray _normals) const
{
    ...
    // Note: origin of volume coordinate is placed
    // in the center of voxel (0,0,0), not in the corner of the voxel!
    Vec4f boxMin, boxMax(volSize.x - voxelSize, volSize.y - voxelSize, volSize.z - voxelSize);
    Vec2f finv(r.fxinv, r.fyinv), cxy(r.cx, r.cy);
    float tstep = truncDist * raycastStepFactor;

    Vec4i volResGpu(volResolution.x, volResolution.y, volResolution.z);

    k.args(ocl::KernelArg::WriteOnlyNoSize(points), ocl::KernelArg::WriteOnlyNoSize(normals),
           frameSize, ocl::KernelArg::PtrReadOnly(volume), ocl::KernelArg::PtrReadOnly(vol2camGpu),
           ocl::KernelArg::PtrReadOnly(cam2volGpu), finv.val, cxy.val, boxMin.val, boxMax.val,
           tstep, voxelSize, volResGpu.val, volDims.val, neighbourCoords.val);
...
void TSDFVolumeGPU::fetchNormals(InputArray _points, OutputArray _normals) const
{
    ...
    if (_normals.needed())
    {
        ...
        k.args(ocl::KernelArg::ReadOnlyNoSize(points), ocl::KernelArg::WriteOnlyNoSize(normals),
               frameSize, ocl::KernelArg::PtrReadOnly(volume),
               ocl::KernelArg::PtrReadOnly(volPoseGpu), ocl::KernelArg::PtrReadOnly(invPoseGpu),
               voxelSizeInv, volResGpu.val, volDims.val, neighbourCoords.val);
...
void TSDFVolumeGPU::fetchPointsNormals(OutputArray points, OutputArray normals) const
{
    ...
    if (points.needed())
    {
        ...
        kscan.args(ocl::KernelArg::PtrReadOnly(volume), volResGpu.val, volDims.val,
                   neighbourCoords.val, ocl::KernelArg::PtrReadOnly(volPoseGpu), voxelSize,
                   voxelSizeInv, ocl::KernelArg::Local(lszscan),
        ...
        if (gpuSum)
        {
            ...
            kfill.args(ocl::KernelArg::PtrReadOnly(volume), volResGpu.val, volDims.val,
                       neighbourCoords.val, ocl::KernelArg::PtrReadOnly(volPoseGpu), voxelSize,
                       voxelSizeInv, ((int)needNormals), ocl::KernelArg::Local(lszfill),
                       ocl::KernelArg::PtrReadWrite(atomicCtr),
                       ocl::KernelArg::ReadOnlyNoSize(groupedSum),
                       ocl::KernelArg::WriteOnlyNoSize(pts), ocl::KernelArg::WriteOnlyNoSize(nrm));
...
cv::Ptr<TSDFVolume> makeTSDFVolume(Point3i _res, float _voxelSize, cv::Affine3f _pose,
                                   float _truncDist, int _maxWeight, float _raycastStepFactor)
{
#ifdef HAVE_OPENCL
    if (cv::ocl::useOpenCL())
        return cv::makePtr<TSDFVolumeGPU>(_res, _voxelSize, _pose, _truncDist, _maxWeight,
                                          _raycastStepFactor);
#endif
    return cv::makePtr<TSDFVolumeCPU>(_res, _voxelSize, _pose, _truncDist, _maxWeight,
                                      _raycastStepFactor);
}
