...
struct DepthWriter
{
    DepthWriter(string fileList) : file(fileList, ios::out), count(0), dir()
    {
        size_t slashIdx = fileList.rfind('/');
        slashIdx        = slashIdx != std::string::npos ? slashIdx : fileList.rfind('\\');
        dir             = fileList.substr(0, slashIdx);
    ...
    void updateParams(Params& params)
    {
        if (vc.isOpened())
        {
            ...
            if (sourceType == Type::DEPTH_REALSENSE)
            {
                ...
                params.volumePose =
                    Affine3f().translate(Vec3f(-cubeSize / 2.f, -cubeSize / 2.f, 0.05f));
            ...
            if (sourceType == Type::DEPTH_KINECT2)
            {
                ...
                initUndistortRectifyMap(camMatrix, distCoeffs, cv::noArray(), camMatrix, frameSize,
