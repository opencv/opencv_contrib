    ...
    void updateParams(large_kinfu::Params& params)
    {
        if (vc.isOpened())
        {
            ...
            if(sourceType == Type::DEPTH_REALSENSE)
            {
                // all sizes in meters
                float cubeSize = 1.f;
                params.volumeParams.voxelSize = cubeSize/params.volumeParams.resolution[0];
                params.volumeParams.tsdfTruncDist = 0.01f;
                params.icpDistThresh = 0.01f;
                params.volumeParams.pose = Affine3f().translate(Vec3f(-cubeSize/2.f,
                                                               -cubeSize/2.f,
                                                               0.05f));
