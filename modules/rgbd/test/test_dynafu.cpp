...
void flyTest(bool hiDense, bool inequal)
{
    ...
    Affine3f startPoseGT = poses[0], startPoseDF;
    Affine3f pose, dfPose;
    for(size_t i = 0; i < poses.size(); i++)
    {
        ...
        ASSERT_TRUE(df->update(depth));

        dfPose = df->getPose();
        if(i == 0)
            startPoseDF = dfPose;

        pose = (  startPoseGT.inv() * pose  )*startPoseDF;
    ...
    ASSERT_LT(cv::norm(dfPose.rvec() - pose.rvec()), rvecThreshold);
...
#ifdef HAVE_OPENCL
#ifdef OPENCV_ENABLE_NONFREE
TEST( DynamicFusion, OCL )
#else
TEST(DynamicFusion, DISABLED_OCL)
#endif
{
