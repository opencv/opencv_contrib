...
INSTANTIATE_TEST_CASE_P(Set1, SRMTest,
    ::testing::Combine(
        ::testing::Values("cv/ximgproc/image.png"),
...
TEST(SRMEdgeCases, UniformImageDoesNotCrash)
{
    ...
    Mat result;
    ASSERT_NO_THROW(
        ximgproc::segmentation::SRMSegmentation(uniform, result)
    );
    EXPECT_FALSE(result.empty());
}
...
TEST(SRMEdgeCases, SinglePixelDoesNotCrash)
{
    ...
    Mat result;
    ASSERT_NO_THROW(
        ximgproc::segmentation::SRMSegmentation(single, result)
    );
    EXPECT_FALSE(result.empty());
}
...
TEST(SRMEdgeCases, ZeroQIsClampedSafely)
{
    ...
    Mat result;
    ASSERT_NO_THROW(
        ximgproc::segmentation::SRMSegmentation(img, result, 0)
    );
    EXPECT_FALSE(result.empty());
}
...
TEST(SRMEdgeCases, ZeroSigmaIsClampedSafely)
{
    ...
    Mat result;
    ASSERT_NO_THROW(
        ximgproc::segmentation::SRMSegmentation(img, result, 32, 0.0)
    );
    EXPECT_FALSE(result.empty());
}
