...
void wrapCos(InputArray img, OutputArray out) {
    ...
    for (auto i = 0; i < img_.rows; i++) {
        for (auto j = 0; j < img_.cols; j++) {
            float x = img_.at<float>(i, j) - M_PI_2;
            while (abs(x) > M_PI_2) {
                x += ((x > 0) - (x < 0)) * (M_PI - 2 * abs(x));
            }
            out_.at<float>(i, j) = -x;
...
void lowPassFilter(InputArray image, OutputArray out, int filterSize) {
    ...
    lowPass(Rect_<int>(0.5*rows-filterSize,0.5 * cols - filterSize,
                      0.5*rows+filterSize,0.5 * cols + filterSize)) = 1;
...
void highPassFilter(InputArray image, OutputArray out, int filterSize) {
    ...
    highPass(Rect_<int>(0.5*rows-filterSize,0.5 * cols - filterSize,
                      0.5*rows+filterSize,0.5 * cols + filterSize)) = 0;
