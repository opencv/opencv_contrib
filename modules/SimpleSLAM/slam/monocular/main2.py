...
def _build_parser() -> argparse.ArgumentParser:
    ...
    #  PnP / map-maintenance
    p.add_argument('--pnp_min_inliers', type=int, default=5)
...
def track_with_pnp(
    ...
    frame_no, Twc_prev, Twc_pred, world_map, args
):
    """
    Continuous PnP tracking: project map landmarks with Twc_pred, associate to
    ...
    assoc_tuple : (obj_pids, kp_ids, inlier_mask) for the final pose
    """
    ...
    # ---------- Args / defaults ----------
    margin       = float(getattr(args, "proj_margin", 20.0))     # px
    min_depth    = float(getattr(args, "min_depth", 0.1))        # m
    max_depth    = float(getattr(args, "max_depth", np.inf))     # m
    proj_radius  = float(getattr(args, "proj_radius", 6.0))      # px (at current scale)
    pnp_min_inl  = int(getattr(args, "pnp_min_inliers", 15))
    ransac_thr   = float(getattr(args, "ransac_thresh", 2.0))    # px
    # descriptor thresholds (optional; auto-selected by dtype)
    desc_cos_min = float(getattr(args, "desc_cos_min", 0.70))    # for float descriptors
    hamm_max     = int(getattr(args, "hamm_max", 45))            # for binary descriptors
    use_desc_chk = bool(getattr(args, "use_desc_check", True))   # gate by descriptor if possible

    H, W = img2.shape[:2]
    kp_xy = np.float32([kp.pt for kp in kp_cur])                  # (K,2)

    # -------------------- 1) Visibility gates (predicted pose) --------------------
    # Use Twc_pred to judge visibility (cheirality + frustum + light range band)
    proj_px_pred = project_points(K, Twc_pred, pts_w)            # (N,2)
    T_cw_pred = np.linalg.inv(Twc_pred)
    R_cw_pred, t_cw_pred = T_cw_pred[:3, :3], T_cw_pred[:3, 3]
    Xc_pred = (R_cw_pred @ pts_w.T + t_cw_pred[:, None]).T       # (N,3)
    z_pred  = Xc_pred[:, 2]

    u, v = proj_px_pred[:, 0], proj_px_pred[:, 1]
    valid_pix   = np.isfinite(u) & np.isfinite(v) & \
                  (u >= margin) & (u < W - margin) & \
                  (v >= margin) & (v < H - margin)
    valid_depth = (z_pred > min_depth) & (z_pred < max_depth)

    # Range band relative to predicted camera center
    C_pred = Twc_pred[:3, 3]
    d = np.linalg.norm(pts_w - C_pred[None, :], axis=1)
    # derive a robust band from the distribution (avoid 0/inf pathologies)
    d_med = np.median(d[np.isfinite(d) & (d > 0)])
    if not np.isfinite(d_med) or d_med <= 0:
        d_med = np.median(d) if np.median(d) > 0 else 1.0
    dmin = getattr(args, "range_band_min_ratio", 0.5) * d_med
    dmax = getattr(args, "range_band_max_ratio", 3.0) * d_med
    valid_range = (d >= dmin) & (d <= dmax)

    keep_idx = np.where(valid_pix & valid_depth & valid_range)[0]
    if keep_idx.size < 4:
        print(f"[PNP] Too few gated points (keep={keep_idx.size}) at frame {frame_no}")
        return False, None, set()

    pts_w   = pts_w[keep_idx]
    proj_px = proj_px_pred[keep_idx]
    mp_ids  = [mp_ids[i] for i in keep_idx]

    # -------------------- Descriptor helpers (optional) --------------------
    def _lm_descriptor(pid):
        p = world_map.points[pid]
        # Try common fields/methods; fall back to None
        if hasattr(p, "last_descriptor"):
            try:
                return p.last_descriptor()
            except Exception:
                pass
    ...
    def _desc_match_ok(d_map, d_cur):
        if d_map is None or d_cur is None:
            return True  # no descriptor to check; allow
        d_map = np.asarray(d_map).reshape(-1)
        d_cur = np.asarray(d_cur).reshape(-1)
        if d_map.dtype == np.uint8 or d_cur.dtype == np.uint8:
            # binary (e.g., ORB)
            dist = cv2.norm(d_map, d_cur, cv2.NORM_HAMMING)
            return dist <= hamm_max
        # float: use cosine similarity
        num = float(np.dot(d_map, d_cur))
        den = float(np.linalg.norm(d_map) * np.linalg.norm(d_cur) + 1e-8)
        cos = num / den
        return cos >= desc_cos_min

    # -------------------- Association: windowed NN (+ optional descriptor) --------------------
    def _associate(search_rad_px: float):
        used_kp = set()
        obj_pts, img_pts = [], []
        obj_pids, kp_ids = [], []
        r2 = float(search_rad_px * search_rad_px)

        for i, (u_i, v_i) in enumerate(proj_px):
            # Greedy NN in image space
            d2 = np.sum((kp_xy - (u_i, v_i))**2, axis=1)
            j = int(np.argmin(d2))
            if d2[j] >= r2 or j in used_kp:
                continue
            # Optional descriptor verification (if available)
            if use_desc_chk:
                lm_desc = _lm_descriptor(mp_ids[i])
                cur_desc = None
                try:
                    cur_desc = desc_cur[j]
                except Exception:
                    cur_desc = None
                if not _desc_match_ok(lm_desc, cur_desc):
                    continue

            obj_pts.append(pts_w[i])
            img_pts.append(kp_xy[j])
            obj_pids.append(mp_ids[i])   # landmark id
            kp_ids.append(j)             # keypoint index
            used_kp.add(j)
    ...
    pts3d, pts2d, obj_pids, kp_ids = _associate(proj_radius)
    if len(pts3d) < max(4, pnp_min_inl // 2):
        pts3d, pts2d, obj_pids, kp_ids = _associate(proj_radius * 1.5)
    ...
    # -------------------- 3) Inliers under refined pose (add cheirality) --------------------
    rvec, _ = cv2.Rodrigues(R_cw)
    ...
    # cheirality under refined pose
    T_cw_cur = np.linalg.inv(Twc_cur)
    R_cw_cur, t_cw_cur = T_cw_cur[:3, :3], T_cw_cur[:3, 3]
    Xc_cur = (R_cw_cur @ pts3d.T + t_cw_cur[:, None]).T
    z_cur = Xc_cur[:, 2]

    inlier_mask = (reproj_err < ransac_thr) & (z_cur > min_depth)
    num_inl = int(np.count_nonzero(inlier_mask))
    if num_inl < pnp_min_inl:
        print(f"[PNP] Too few inliers after refine ({num_inl}<{pnp_min_inl}) at frame {frame_no}")
        return False, None, set()

    # -------------------- 4) One re-association pass with refined pose --------------------
    # Recompute gating with Twc_cur; then associate in a smaller window.
    proj_px_cur = project_points(K, Twc_cur, pts_w)          # note: pts_w is the gated subset
    T_cw_cur_full = np.linalg.inv(Twc_cur)
    Xc_cur_full = (T_cw_cur_full[:3, :3] @ pts_w.T + T_cw_cur_full[:3, 3][:, None]).T
    z_cur_full = Xc_cur_full[:, 2]
    u2, v2 = proj_px_cur[:, 0], proj_px_cur[:, 1]
    valid_pix2 = np.isfinite(u2) & np.isfinite(v2) & \
                 (u2 >= margin) & (u2 < W - margin) & \
                 (v2 >= margin) & (v2 < H - margin)
    valid_depth2 = (z_cur_full > min_depth) & (z_cur_full < max_depth)
    keep2 = np.where(valid_pix2 & valid_depth2)[0]
    if keep2.size >= 4:
        pts_w2   = pts_w[keep2]
        proj_px2 = proj_px_cur[keep2]
        mp_ids2  = [mp_ids[i] for i in keep2]

        # Temporarily swap the working sets for a tighter association
        pts_w_save, proj_px_save, mp_ids_save = pts_w, proj_px, mp_ids
        pts_w, proj_px, mp_ids = pts_w2, proj_px2, mp_ids2

        pts3d_b, pts2d_b, obj_pids_b, kp_ids_b = _associate(max(2.0, proj_radius * 0.5))
        if len(pts3d_b) >= 4:
            R_cw_b, tvec_b = refine_pose_pnp(K, pts3d_b, pts2d_b)
            if R_cw_b is not None:
                T_cw_b = _pose_rt_to_homogenous(R_cw_b, tvec_b)
                Twc_cur_b = _pose_inverse(T_cw_b)

                # Inliers for the second pass (with cheirality)
                rvec_b, _ = cv2.Rodrigues(R_cw_b)
                proj_b, _ = cv2.projectPoints(pts3d_b.astype(np.float32), rvec_b, tvec_b, K, None)
                proj_b = proj_b.reshape(-1, 2)
                err_b = np.linalg.norm(proj_b - pts2d_b, axis=1)

                T_cw_b_full = np.linalg.inv(Twc_cur_b)
                Xc_b = (T_cw_b_full[:3, :3] @ pts3d_b.T + T_cw_b_full[:3, 3][:, None]).T
                z_b  = Xc_b[:, 2]
                mask_b = (err_b < ransac_thr) & (z_b > min_depth)
                if int(np.count_nonzero(mask_b)) >= max(num_inl, pnp_min_inl):
                    # Accept tighter solution
                    Twc_cur = Twc_cur_b
                    pts3d, pts2d, obj_pids, kp_ids = pts3d_b, pts2d_b, obj_pids_b, kp_ids_b
                    inlier_mask = mask_b
                    num_inl = int(np.count_nonzero(inlier_mask))

        # restore working sets
        pts_w, proj_px, mp_ids = pts_w_save, proj_px_save, mp_ids_save

    print(f"[PNP] Pose @ frame {frame_no} refined with {num_inl} inliers "
          f"(thr={ransac_thr:.2f}, margin={margin:.1f}, radius={proj_radius:.1f})")
    ...
    try:
        ...
        print(f"[PNP] Viz error (ignored): {e}")

    # Return the triple (to match your current caller usage)
...
def main():
    ...
    for i in tqdm(range(total), desc='Tracking'):
        ...
        # TODO constant-velocity prediction
        # Twc_pred = Twc_cur_pose  # fallback: last pose
        # if i >= 2:
        #     Twc_prevprev = world_map.poses[-2] if len(world_map.poses) >= 2 else None
        #     if Twc_prevprev is not None:
        #         T_rel = np.linalg.inv(Twc_prevprev) @ Twc_cur_pose    # c_{t-2} -> c_{t-1}
        #         Twc_pred = Twc_cur_pose @ T_rel                        # predict c_t
        Twc_pred = Twc_cur_pose
        if Twc_prev_frame is not None and Twc_prevprev_frame is not None:
            T_rel = np.linalg.inv(Twc_prevprev_frame) @ Twc_prev_frame   # c_{t-2} -> c_{t-1}
            Twc_pred = Twc_prev_frame @ T_rel   
        
        ok_pnp, Twc_cur_pose, assoc = track_with_pnp(
            K, kp1, kp2, des1, des2, matches, frame_no=i+1, img2=img2,
            Twc_prev=Twc_cur_pose, Twc_pred=Twc_pred, world_map=world_map, args=args)
