...
class StructureFromMotion:
    def __init__(self, prefix, thumpup_pos_thresh, thumpup_rot_thresh):
        # You can set this to True if you definitely want to run stereo mode.
        # If the dataset doesn't have right-camera images, you can set it to False.
        self.isStereo = False
        ...
        if 'kitti' in prefix:
            ...
            # Set stereo to True if you want to use stereo images. 
            # If the dataset is incomplete, set it to False.
            # self.isStereo = True
            ...
            # For parking dataset, typically no stereo images are provided,
            # so you can set self.isStereo = False if needed.
            ...
            For a custom dataset:
             - we expect:
                prefix/
                  custom_video.mp4 or custom_compress.mp4 (video frames)
                  calibration.pkl (something that provides camera intrinsics)
            ...
            self.image_list = list(range(len(self.frames)))  # e.g. [0..N-1]

            # 3) Optionally load ground truth
        ...
        # =============== STEREO SGBM SETUP if self.isStereo = True =========
        if self.isStereo:
            self.MIN_DISP = 0
            self.NUM_DISP = 32
            self.BLOCK_SIZE = 11
    ...
    def malaga_get_gt(self, filepath, gt_filepath):
        ...
        np.save(gt_filepath + ".npy", gt)
        np.savetxt(gt_filepath + ".txt", gt.reshape(gt.shape[0], -1), fmt="%.5f")
        return gt
    ...
    def triangulate(self, R, t, pos_m_kp1, pos_m_kp2):
        """
        We use self.Proj as the 3×4 matrix for the left camera.
        For the second camera, we do self.Proj @ [R|t].
        """
        P1 = self.Proj
        P2 = self.Proj @ self.get_4by4_homo_proj(R, t)
        points_4d = cv2.triangulatePoints(P1, P2, pos_m_kp1.T, pos_m_kp2.T)
    ...
    def initialize(self, keyframe_filename):
        ...
        for id_n_keyframe in tqdm(range(1, len(self.image_list))):
            ...
            if self.isStereo:
                # For stereo, we load the right images as well:
                img1_r = cv2.imread(self.image_list_r[id_c_keyframe])
                img2_r = cv2.imread(self.image_list_r[id_n_keyframe])
            ...
            if initial_frame or not self.isStereo:
                ...
                # Triangulate in the left-right images for both frames
    ...
    def run(self, keyframe_filename, checkpoints=20, dynamic_vis=True):
        """
        Runs the main pipeline using either the keyframes from file
        or all frames if no file is found. The 'dynamic_vis' parameter
        can be set to False if you do not want interactive visualization.
        """
        import matplotlib.pyplot as plt

        # 1) Load or generate keyframes
        if os.path.exists(keyframe_filename):
        ...
        # 2) Conditionally set up visualization
        vis_param = None
        if dynamic_vis:
            vis_param = self.vis_init()
        ...
        for i in tqdm(range(1, iter_count)):
            ...
            # If stereo is active, compute disparities or do pnp
            if self.isStereo:
            ...
            # --- If ground truth is available, compute any desired loss
            ate_loss = -1
            rte_loss = -1
            if (self.gt is not None) and (idx2 < self.gt.shape[0]):
                ...
                # For RTE: 
                # if i >= 2 and (idx1 < self.gt.shape[0]):
                #     rte_loss = self.compute_rte(all_poses[-2], all_poses[-1], self.gt[idx1], self.gt[idx2])
