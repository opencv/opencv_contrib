...
void LineBinarizer::binarizeImage(const unsigned char *src, unsigned char *dst, int width,
                                  int height) {
    ...
    for (int j = 0; j < norm_roi.rows; j++) {
        ...
        binarizeLine(src_line, dst_line, width);
    }

    return;
}

bool LineBinarizer::binarizeLine(const unsigned char *line, unsigned char *dst, int width) {
    std::vector<short> maxiam_index;
    std::vector<short> miniam_index;
    scanLine(line, width, maxiam_index, miniam_index);
...
void LineBinarizer::scanLine(const unsigned char *line, int width, std::vector<short> &maxiam_index,
                             std::vector<short> &miniam_index) {
    short cur_max_i = 0;
    short cur_min_i = 0;
