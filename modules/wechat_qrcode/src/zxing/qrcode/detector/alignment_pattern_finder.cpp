...
AlignmentPatternFinder::AlignmentPatternFinder(Ref<BitMatrix> image, int startX, int startY,
    ...
    : image_(image),
      ...
      possibleWhiteCenters_(new vector<AlignmentPattern *>()),
      lastTry(false)
#endif
{
}
...
std::vector<Ref<AlignmentPattern> > AlignmentPatternFinder::find(ErrorHandler &err_handler) {
    ...
    for (int iGen = 0; iGen < height_; iGen++) {
                ...
                if (currentState == 1) {  // Counting black pixels
                    ...
                    if (currentState == 2) {  // A winner?
                        ...
                        if (foundAlignmentPattern || foundWhiteAlignmentPattern) {
                            ...
                            // confirmed(handlePossibleCenter(stateCount, i,
                            // j)); if (confirmed != 0) {
                            //	return confirmed;
                            //}
                            handlePossibleCenter(stateCount, i, j);
                            vector<Ref<AlignmentPattern> > results;
                            if (confirmedAlignmentPattern != 0 &&
                                confirmedWhiteAlignmentPattern != 0) {
        ...
        if (foundAlignmentPattern || foundWhiteAlignmentPattern) {
            ...
            // i, maxJ)); if (confirmed != 0) { 	return confirmed;
            //}
            vector<Ref<AlignmentPattern> > results;
            handlePossibleCenter(stateCount, i, j);
...
float AlignmentPatternFinder::centerFromEndWhite(vector<int> &stateCount, int end) {
    return (float)(end - stateCount[2] - stateCount[1]) - stateCount[0] / 2.0f;
}
#endif
...
void AlignmentPatternFinder::foundPatternCross(vector<int> &stateCount) {
    ...
    foundWhiteAlignmentPattern = false;
    if (abs(3.0 * moduleSize_ - stateCount[0]) <= moduleSize_) foundWhiteAlignmentPattern = true;
...
#ifdef FIND_WHITE_ALIGNMENTPATTERN
float AlignmentPatternFinder::crossCheckVerticalWhite(int startI, int centerJ, int maxCount,
                                                      int originalStateCountTotal) {
    // This is slightly faster than using the Ref. Efficiency is important here
    BitMatrix &matrix = *image_;
    int maxI = matrix.getHeight();
...
void AlignmentPatternFinder::handlePossibleCenter(vector<int> &stateCount, int i, int j) {
    ...
    if (foundAlignmentPattern) {
        ...
        centerI = crossCheckVertical(i, (int)centerJ, 2 * stateCount[1], stateCountTotal);
        if (!isnan(centerI)) {
    ...
    if (foundWhiteAlignmentPattern) {
        ...
        centerI = crossCheckVerticalWhite(i, (int)centerJ, stateCount[0] * 3, stateCount[0] * 3);
        if (!isnan(centerI)) {
            estimatedModuleSize = (float)stateCount[0] / 3.0f;
            ...
            for (int index = 0; index < max; index++) {
                ...
                if (center->aboutEquals(estimatedModuleSize, centerI, centerJ)) {
                    //	return center->combineEstimate(centerI, centerJ,
                    // estimatedModuleSize);
                    confirmedWhiteAlignmentPattern =
                        center->combineEstimate(centerI, centerJ, estimatedModuleSize);
                }
            }
            AlignmentPattern *tmp = new AlignmentPattern(centerJ, centerI, estimatedModuleSize);
...
bool AlignmentPatternFinder::foundPatternCross(int *stateCount) {
    ...
    for (int i = 1; i < 4; i++) {
        ...
        if (antiOutOfFocusCount > 2) return false;
    }
    return true;
}
...
void AlignmentPatternFinder::handlePossibleCenter(int *stateCount, size_t i, size_t j) {
    ...
    float centerJ = centerFromEnd(stateCount, j);
    float centerI =
        crossCheckVertical(i, (size_t)centerJ, stateCountTotal, stateCountTotal, verticalState);
    if (!isnan(centerI)) {
        // Re-cross check
        stateCountTotal = verticalState[1] + verticalState[2] + verticalState[3];
        centerJ = crossCheckHorizontal((size_t)centerJ, (size_t)centerI, stateCountTotal,
                                       stateCountTotal, horizontalState);
        if (!isnan(centerJ)) {
            int max = possibleCenters_.size();
            ...
            for (int index = 0; index < max; index++) {
                Ref<AlignmentPattern> center(possibleCenters_[index]);
                // Look for about the same center and module size:
                if (center->aboutEquals(centerI, centerJ, verticalModuleSize,
                                        horizontalModuleSize)) {
                    center->combineEstimate(centerI, centerJ, verticalState, horizontalState);
                    return;
                }
            }
            Ref<AlignmentPattern> newPattern(
                new AlignmentPattern(centerJ, centerI, horizontalState, verticalState));
...
AlignmentPatternFinder::AlignmentPatternFinder(Ref<BitMatrix> image, float moduleSize)
    : image_(image), moduleSize_(moduleSize) {}

bool AlignmentPatternFinder::findPatternLine(Ref<FinderPattern> const &from,
                                             Ref<FinderPattern> const &to, double &aValue,
                                             double &pValue) {
...
bool AlignmentPatternFinder::findPatternLineSample(Ref<FinderPattern> const &from,
                                                   Ref<FinderPattern> const &to,
                                                   vector<pair<int, int> > &sample) {
    int startX, startY;
    bool steep;
    int xstep;
    if (!findPatternLineStartPoint(from, to, startX, startY, steep, xstep)) return false;
    ...
    if (steep) {
        ...
        beginY = (int)((float)startY - verticalState[2] / 2.0f - verticalState[3]);
        endY = (int)((float)startY + verticalState[2] / 2.0f + verticalState[3]);
    } else {
    ...
    findPatternLineSampleOneSide(startX, startY, beginY + 1, (int)(deltaX + 0.5f), steep, -1,
                                 sample);
    findPatternLineSampleOneSide(startX, startY + 1, endY + 1, (int)(deltaX + 0.5f), steep, 1,
                                 sample);

    int suitable = (int)(0.7f * (endY - beginY + 1));
    if ((int)sample.size() < suitable) {
        return false;
    }
...
void AlignmentPatternFinder::findPatternLineSampleOneSide(
    int startX, int startY, int endY, int deltaX, int steep, int side,
...
bool AlignmentPatternFinder::findPatternLineStartPoint(Ref<FinderPattern> const &from,
                                                       Ref<FinderPattern> const &to, int &startX,
                                                       int &startY, bool &steep, int &xstep) {
    ...
    int dx = (int)fabs(toX - fromX);
    int dy = (int)fabs(toY - fromY);
    int error = -dx >> 1;
    xstep = fromX < toX ? 1 : -1;
    ...
    int xLimit = (int)(toX + xstep);
    for (int x = (int)fromX, y = (int)fromY; x != xLimit; x += xstep) {
        int realX, realY;
        if (steep) {
            realX = y;
            realY = x;
        ...
        switch (state) {
            case 0:
                if (!image_->get(realX, realY)) ++state;
                break;
            case 1:
...
int AlignmentPatternFinder::findPatternLineCenterOneSide(int startX, int startY, int deltaX,
                                                         bool steep, int side) {
    int x, y, i;
    int range = (int)(1.5f * deltaX);
    for (i = 0; i <= range; ++i) {
        if (steep) {
            x = startY;
            y = startX + i * side;
    ...
    if (i < 0.8f * deltaX) {
        ...
        for (i = beginI; i > 0; --i) {
            ...
            if (x < 0 || (size_t)x >= image_->getWidth() || y < 0 ||
                (size_t)y >= image_->getHeight()) {
                if (i == beginI)
                    i = beforeI;
                else if (i > 0)
                    --i;
...
float AlignmentPatternFinder::crossCheckVertical(size_t startI, size_t centerJ, int maxCount,
    ...
    if (variance > maxVariance || variance < 1.0 / maxVariance) return nan();
...
Ref<AlignmentPattern> AlignmentPatternFinder::findInRange(
    Ref<AlignmentPattern> const &estimateCenter, size_t startX, size_t startY, size_t width,
    size_t height, ErrorHandler &err_handler) {
    size_t maxJ = startX + width;
    size_t middleI = startY + (height >> 1);
    //      Ref<BitArray> luminanceRow(new BitArray(width));
    // We are looking for white/black/black modules in 1:1:1 ratio;
    // this tracks the number of black/white/black modules seen so far
                ...
                case 4:
                    if (image_->get(j, i)) {
                        ...
                        /*
                           ...
                           cout << "foundPatternCross falure, x:\t" << j << "
                           y:\t" << i << endl; cout << "stateCount:\t"; for (int
                           k = 0; k < 5; ++k) cout << stateCount[k] << " "; cout
                           << endl;
                           }*/
                        stateCount[0] = stateCount[2];
    ...
    if (possibleCenters_.size() > 0) {
        ...
        for (size_t i = 0; i < possibleCenters_.size(); ++i) {
            ...
            if (fix >= 90.0f) {
                ...
                maxFixIndex = i;
                break;
            } else if (possibleCenters_[i]->aboutEquals(
                           estimateCenter->getY(), estimateCenter->getX(),
                           estimateCenter->getEstimatedVerticalModuleSize(),
                           estimateCenter->getEstimatedHorizontalModuleSize()) &&
                       fix >= 70.0f) {
                maxFix = fix;
                maxFixIndex = i;
                break;
        ...
        // return (*possibleCenters_)[maxFixIndex];
        return Ref<AlignmentPattern>(new AlignmentPattern(
            (possibleCenters_[maxFixIndex])->getX(), (possibleCenters_[maxFixIndex])->getY(),
            (possibleCenters_[maxFixIndex])->getEstimatedModuleSize()));
    }
