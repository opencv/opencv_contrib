...
def model_download(filename, url, sha1):
    print "Downloading '{}' to {} from {} ...".format(model_name, dst_file, model["url"])
...
def post_download_action(filepath, action):
    if action == "unzip":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Downloading extra data.")
    ...
    try:
        ...
        print "ERROR: Can't parse {}".format(args.download_list)
        sys.exit(1)
    ...
    for model_name in models_to_download:
        ...
        if os.path.exists(dst_file) and model_checks_out(dst_file, model["sha1"]):
            print("Model {} already exists.".format(dst_file))
        else:
            model_download(dst_file, model["url"], model["sha1"])
            if "action" in model.keys():
                post_download_action(dst_file, model["action"])
