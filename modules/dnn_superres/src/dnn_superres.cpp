...
namespace cv
{
    namespace dnn
    {
        namespace dnn_superres
        {
            ...
            void DnnSuperResImpl::upsample(Mat img, Mat &img_new)
            {
                if( !net.empty() )
                {
                    if (this->alg == "espcn")
                    {
                        ...
                        Mat preproc_img;
                        preprocessESPCN(img, preproc_img);
                        ...
                        reconstructESPCN(out_img, preproc_img, img_new);
                    }
                    else
                    {
                        //get blob
            ...
            void DnnSuperResImpl::preprocessESPCN(const Mat inpImg, Mat &outImg)
            {
            ...
            void DnnSuperResImpl::reconstructESPCN(const Mat inpImg, const Mat origImg, Mat &outImg)
            {
                if (origImg.type() == CV_32FC3)
                {
                    ...
                    Mat Cr, Cb;
                    cv::resize(orig_channels[1], Cr, cv::Size(), this->sc, this->sc);
                    cv::resize(orig_channels[2], Cb, cv::Size(), this->sc, this->sc);
