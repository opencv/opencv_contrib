...
def init_tracker(tracker_name):
    ...
    return [tracker, frames_before_reinit]
...
def main():
    ...
    trackers = {'Boosting': 500, 'MIL': 1000, 'KCF': 110,
                'MedianFlow': 20, 'GOTURN': 20, 'MOSSE': 20, 'CSRT': 250}

    #For every tracker
    for tracker_id in range(len(list(trackers.keys()))):

        tracker_name = list(trackers.keys())[tracker_id]
        print(tracker_name)
        ...
        for video_name in list_of_videos:
            ...
            for number_of_the_frame, image in enumerate(video_sequence):
                ...
                # Metrics: Intersection over Union, Precision, Normalized Precision
