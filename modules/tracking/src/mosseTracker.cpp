...
protected:
    ...
    void preProcess(Mat& window) const
    {
        ...
        window = (window - mean[0]) / (StdDev[0] + params.eps);
    ...
    double correlate(const Mat& image_sub, Point& delta_xy) const
    {
        ...
        return (maxVal - mean[0]) / (std[0] + params.eps);  // PSR
    }
    ...
    virtual bool updateImpl(const Mat& image, Rect2d& boundingBox) CV_OVERRIDE
    {
        ...
        // update A ,B, and H
        A = A * (1 - params.rate) + A_new * params.rate;
        B = B * (1 - params.rate) + B_new * params.rate;
        H = divDFTs(A, B);
...
void TrackerMOSSE::Params::read(const cv::FileNode& fn)
{
    *this = TrackerMOSSE::Params();
...
void TrackerMOSSE::Params::write(cv::FileStorage& fs) const
{
    fs << "eps" << eps;
    fs << "rate" << rate;
...
Ptr<TrackerMOSSE> TrackerMOSSE::create(const TrackerMOSSE::Params& params)
{
    return makePtr<tracking::MosseImpl>(params);
}
