...
The CUDA-accelerated `cuda*` modules in this repository can also be built for AMD GPUs through HIP. Configure the core `opencv` build with `-DWITH_HIP=ON` (instead of `-DWITH_CUDA=ON`) and, optionally, `-DCMAKE_HIP_ARCHITECTURES=<gfx targets>`; the ROCm toolkit must be installed. The same `cv::cuda` API is exposed and the modules are compiled with the ROCm HIP compiler. A small set of entry points (the NVIDIA hardware optical-flow classes and graph-cut stereo) have no ROCm analog and report `cv::Error::StsNotImplemented`; the corresponding software algorithms remain available. See the [build configuration reference](https://docs.opencv.org/master/db/d05/tutorial_config_reference.html) for details.
